...
use libp2p::{
    PeerId, gossipsub, identify, identity, kad, mdns, noise,ping,
dns,    swarm::{NetworkBehaviour, SwarmEvent},
    tcp, yamux,
};
use std::fs;
use std::path::Path;
...
#[tokio::main]
async fn main() -> Result<(), Box<dyn Error>> {
    let path = Path::new("/home/sansh2356/Desktop/simple_p2p/example_2/src/key_pair.gpg");
    let fetched_value =
